...
class SignupTest(unittest.TestCase):
    ...
    def test_signup(self):
        ...
        response = self.client.put("/signup", data={'username' : 'betty', 'password' : '456'})
        resp_json = response.data
        resp = json.loads(resp_json)
        self.assertEqual(resp, "success")

        # test whether sign up is a success
        response = self.client.get("/login", data={'username': 'betty', 'password': '456'})
        resp_json = response.data
        resp = json.loads(resp_json)
...
class AddHouseInfoTest(unittest.TestCase):
    ...
    def test_add_house_info(self):
        ...
        response = self.client.post("/add/343 Gold Street+www.housing.com", data={})
        resp_json = response.data
        resp = json.loads(resp_json)
        self.assertEqual(resp, "success")
